		...
		go func(c net.Conn) {
			...
			if err := decoder.Decode(&req); err != nil {
				if errors.Is(err, io.EOF) {
					log.Printf("client disconnected before request read: remote=%s err=%v", c.RemoteAddr(), err)
				} else {
					log.Printf("request decode failed: remote=%s err=%v", c.RemoteAddr(), err)
				}
				return
			}
			...
			if err := encoder.Encode(resp); err != nil {
				log.Printf("failed to send response: remote=%s request_id=%s method=%s err=%v", c.RemoteAddr(), req.RequestID, req.Method, err)
				return
			}
